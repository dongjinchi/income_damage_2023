...
reghdfe dgdi c.dT c.dT#c.tem  c.dP c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
reghdfe dgdi c.tem##c.tem c.pre##c.pre edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
reghdfe dgdi c.dT c.dT#c.tem  c.dP c.dP#c.pre  c.tem##c.tem c.pre##c.pre  edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
foreach lev of loc levs  {
	foreach var of loc vars {
		reghdfe dgdi c.dT c.dT#c.tem  c.dP  c.dP#c.pre  c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd (c.`lev'##c.`lev')#(c.`var'##c.`var') edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
reghdfe dgdi poor#(c.dT c.dT#c.tem  c.dP  c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd ) edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
reghdfe dgdi c.dT c.dT#c.tem  c.dP  c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd  poor#((c.tem_sd##c.tem_sd)#(c.tem##c.tem)) edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
reghdfe dgdi c.dT c.dT#c.tem  c.dP  c.dP#c.pre  c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd poor#((c.pre_sd##c.pre_sd)#(c.pre##c.pre))   edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
qui reghdfe dgdi dT c.dT#c.tem dP c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd edu  pop,noabsorb vce(cluster country1) res(re)
...
qui reghdfe dgdi dT c.dT#c.tem dP c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd edu  pop,absorb(i.year gdlcode1##c.year) vce(cluster country1) res(ref)
...
qui reghdfe dgdi l.dgdi  dT c.dT#c.tem dP c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd edu  pop,absorb(i.year gdlcode1##c.year) vce(cluster country1) res(refl)
...
reghdfe dgdi c.dT c.dT#c.tem_era  c.dP c.dP#c.pre_era c.tem_era##c.tem_era c.pre_era##c.pre_era c.asp_tem##c.asp_tem c.asp_pre##c.asp_pre pop  edu ,absorb(i.year gdlcode1##c.year) vce( cluster country1)
estimates store r14, title("DTW_type_ERA")
restore

esttab r14 using TableS17.rtf, star(* .1 ** .05  *** .01) nogap nonumber replace se(%5.4f) ar2 aic(%10.4f) bic(%10.4f) //(Table S11)


// Replace ASP by WASP (Table S18)
reghdfe dgdi c.dT c.dT#c.tem  c.dP c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_wsd##c.pre_wsd edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
estimates store r15, title("DTW_type_WASP")

esttab r15 using TableS18.rtf, star(* .1 ** .05  *** .01) nogap nonumber replace se(%5.4f) ar2 aic(%10.4f) bic(%10.4f) //(Table S11)

*plot
//effect of annual mean temperature (Figure 2a)
qui reghdfe dgdi c.dT c.dT#c.tem  c.dP c.dP#c.pre c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
yline(0,lwidth(thin) lcolor(gs10) ) ///
 legend(off)  ytitle(Change in ln(GDI per capital), size(small)) xtitle(Annual mean temperature(℃), size(small)) graphregion(color(white))
...
yline(0,lwidth(thin) lcolor(gs10) ) ///
 legend(off)  ytitle(Margin effect on ln(GDI per capital) change, size(small)) xtitle(Annual mean temperature(℃), size(small)) graphregion(color(white))
...
yline(0,lwidth(thin) lcolor(gs10) ) ///
 legend(off)  ytitle(Change in ln(GDI per capital), size(small)) xtitle(Annual total precipitation(m), size(small)) graphregion(color(white))
...
yline(0,lwidth(thin) lcolor(gs10) ) ///
 legend(off)  ytitle(Margin effect on ln(GDI per capital) change, size(small)) xtitle(Annual total precipitation(m), size(small)) graphregion(color(white))
...
qui reghdfe dgdi c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd c.dT c.dT#c.tem  c.dP  c.dP#c.pre  pop edu,absorb(i.year gdlcode1##c.year) vce( cluster country1)
qui margins,dydx(dT) at( tem_sd=0 pre_sd = 0) atmeans post level(90)
estimates store m1
qui reghdfe dgdi poor#c.(c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd  c.dT c.dT#c.tem c.dP c.dP#c.pre ) edu pop,absorb(i.year gdlcode1##c.year) vce( cluster country1)
qui margins,dydx(dT) at(poor =(0 1) tem_sd=0 pre_sd = 0) atmeans post level(90)
estimates store m2
coefplot ///
(r1, keep(c.tem#c.tem c.tem_sd#c.tem_sd ) mcolor("190 50 190%60") ciopts(color("190 50 190") lwidth(medthick)) ms(O) ) ///
(m1, mcolor("190 50 190%60") ciopts(color("190 50 190") lwidth(medthick)) ms(O) ) /// 
(r8, keep(0.poor#c.tem#c.tem 0.poor#c.tem_sd#c.tem_sd) baselevels level(90) mcolor("190 50 190")  ciopts(color("190 50 190") lwidth(medthick)) ms(Dh)) ///
(r8, keep(1.poor#c.tem#c.tem 1.poor#c.tem_sd#c.tem_sd) mcolor("190 50 190") ciopts(color("190 50 190") lwidth(medthick)) ms(Sh)) ///
(m2, keep(1._at) baselevels level(90) mcolor("190 50 190")  ciopts(color("190 50 190") lwidth(medthick)) ms(Dh)) ///
(m2, keep(2._at) baselevels level(90) mcolor("190 50 190")  ciopts(color("190 50 190") lwidth(medthick)) ms(Sh)) ///
,xline(0,lwidth(thin) lcolor(black) lpattern(dash))  xlabel(-0.004(0.002)0.016, labsize(small) nogrid) 
graph save Graph  "E:\06博士论文\002出国交流\02cliamte_economy\global\plot\GDI_figure4a.png", replace
...
estimates store m4
coefplot ///
(r1, keep(c.pre#c.pre c.pre_sd#c.pre_sd ) mcolor("190 50 190%60") ciopts(color("190 50 190") lwidth(medthick)) ms(O) ) /// 
(m3, mcolor("190 50 190%60") ciopts(color("190 50 190") lwidth(medthick)) ms(O) ) /// 
(r8, keep(0.poor#c.pre#c.pre 0.poor#c.pre_sd#c.pre_sd) baselevels level(90) mcolor("190 50 190")  ciopts(color("190 50 190") lwidth(medthick)) ms(Dh)) ///
(r8, keep(1.poor#c.pre#c.pre 1.poor#c.pre_sd#c.pre_sd) mcolor("190 50 190") ciopts(color("190 50 190") lwidth(medthick)) ms(Sh)) ///
(m4, keep(1._at) baselevels level(90) mcolor("190 50 190")  ciopts(color("190 50 190") lwidth(medthick)) ms(Dh)) ///
(m4, keep(2._at) baselevels level(90) mcolor("190 50 190")  ciopts(color("190 50 190") lwidth(medthick)) ms(Sh)) ///
,xline(0,lwidth(thin) lcolor(black) lpattern(dash))  xlabel(-0.025(0.01)0.035, labsize(small) nogrid) 
graph save Graph  "E:\06博士论文\002出国交流\02cliamte_economy\global\plot\GDI_figure4b.png", replace

//Heterogeneity analysis for the interaction effect of temperature (Figure 4c)
qui reghdfe dgdi c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd c.dT c.dT#c.tem  c.dP  c.dP#c.pre poor#((c.tem_sd##c.tem_sd)#(c.tem##c.tem))   pop edu,absorb(i.year gdlcode1##c.year) vce( cluster country1)
margins, over(poor) at(tem_sd=(-4(0.2)4) pre_sd=0 tem=10) atmeans post level(90)
preserve
parmest, norestore level(90)
...
, xlabel(-4(1)4)  ylabel(-0.5(0.1)0.3,nogrid) ///
 yline(0,lwidth(thin) lcolor(black)) ///
 legend(lab(1 "Rich CI-90%") lab(2 "Poor CI-90%") lab(3 "Rich regions") lab(4 "Poor regions")) xtitle(AST) ytitle(GDI per capital growth) graphregion(color(white))
 graph save Graph "figure\GDI_inter_T10.png",replace
restore

//Heterogeneity analysis for the interaction effect of precipitation (Figure 4e)
qui reghdfe dgdi c.tem##c.tem c.pre##c.pre c.tem_sd##c.tem_sd c.pre_sd##c.pre_sd c.dT c.dT#c.tem  c.dP  c.dP#c.pre poor#((c.pre_sd##c.pre_sd)#(c.pre##c.pre))   pop edu,absorb(i.year gdlcode1##c.year) vce( cluster country1)
...
(line est_rich x if poor ==0 , lcolor("154 4 44" ) ) ///
(line est_poor x if poor ==1 , lcolor("131 80 200") ) ///
, xlabel(-4(1)4)  ylabel(-0.1(0.1)0.4,nogrid) ///
 yline(0,lwidth(thin) lcolor(black)) ///
 legend(lab(1 "Rich CI-90%") lab(2 "Poor CI-90%") lab(3 "Rich regions") lab(4 "Poor regions")) xtitle(ASP) ytitle(GDI per capital growth) graphregion(color(white))
graph save Graph "figure\GDI_inter_P1.2.png",replace
restore